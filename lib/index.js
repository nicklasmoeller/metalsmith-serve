    ...
    server = require('http').createServer(function (request, response) {
      request.addListener('end', function () {
        ...
        fileServer.serve(request, response, function(err, res) {
          if (err) {
            var [destination, statusCode] = normalizeRedirect(options.redirects, request.url)
...
function normalizeRedirect(redirects, url) {
  var match = Object.keys(redirects).find(function(pattern) {
    return minimatch(url, pattern) || new UrlPattern(pattern).match(url)
  })

  var destination = redirects[match]
  var statusCode = 301
  ...
  destination = patternSubstitution(url, match, destination)
